		...
		<div class="add-liquidity">
                        	...
                        	<span class='currency_label'>
                                ...
                                <span v-show='depositc'>{{currencies[currency]}}
    	                        	<span v-show="!(currency == 'usdt' && currentPool == 'usdt' || currency == 'pax')
    	                        					&& !['susdv2', 'tbtc', 'ren', 'sbtc', 'hbtc', 'uSwap','3pool'].includes(currentPool)">
            ...
            <p style="text-align: center" class='buttons'>
                <button id="add-liquidity"
                    :disabled="currentPool == 'susdv2' && slippage < -0.03 || depositingZeroWarning || isZeroSlippage || slippage < -0.1 || (slippage < -0.05 && ['usdt', 'hbtc'].includes(currentPool))"
                ...
                <button id='stakeunstaked'
                    ...
                    @click='stakeTokens()'
                    >
                    Stake unstaked in gauge <span class='loading line' v-show='loadingAction == 3'></span>
                </button>
                <p class='info-message gentle-message' v-show="currentPool == '3pool'">
                    Initial CRV APY boost will be 1x. Boosting up to 2.5x for this gauge starts on {{ formatDateToHuman(1600624709) }} UTC
                </p>
