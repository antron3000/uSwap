<template>
	<div class="window white add-liquidity">
            ...
            <ul>
                <li v-for='(currency, i) in Object.keys(currencies)'>
                    <label :for="'currency_'+i" class='currency_label'>
                    	...
                    	<span v-show="!withdrawc && !['susdv2', 'tbtc', 'ren', 'hbtc'].includes(currentPool)">{{currency | capitalize}}</span>
                        <span v-show="!withdrawc && ['susdv2', 'tbtc', 'ren', 'hbtc'].includes(currentPool)">{{currencies[currency]}}</span>
                    </label>
                    <input type="text" 
                    :id="'currency_'+i" 
                    name="from_cur" 
        ...
        <fieldset v-show = "currentPool != 'susd'">
        	...
        	<ul>
        		<li v-show = "!['susdv2','tbtc','ren','hbtc'].includes(currentPool)">
	            ...
	            <li>
	            	...
	            	<label 
                        for='donate_dust' 
                        v-show="!['tbtc', 'ren','hbtc'].includes(currentPool)">
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            minAmount() {
            if(['tbtc', 'ren', 'hbtc'].includes(currentContract.currentContract)) return 1e-8
                return 0.01
            },
            calcFee() {
        ...
        methods: {
            ...
            toFixed(num, precisions = 2, round = 4) {
                if(precisions == 2 && ['tbtc', 'ren', 'hbtc'].includes(currentContract.currentContract)) precisions = 8
			...
			async handle_remove_liquidity(unstake = false) {
                ...
                if(['tbtc','ren','hbtc'].includes(currentContract.currentContract)) inOneCoin = currentContract.swap
			    ...
			    if (this.share == '---') {
			        ...
			        if(this.withdrawc || this.currentPool == 'susdv2') {
                        ...
                        try {
                            ...
                            if(!['tbtc','ren','hbtc'].includes(currentContract.currentContract)) await common.ensure_allowance_zap_out(token_amount)
                            this.waitingMessage = 'Please confirm withdrawal transaction'
                    ...
                    if(this.to_currency !== null && this.to_currency < 10) {
                        ...
                        if(!['tbtc','ren','hbtc'].includes(currentContract.currentContract)) await common.ensure_allowance_zap_out(amount)
                        let min_amount;
                        try {
                        ...
                        if(!['tbtc','ren','hbtc'].includes(currentContract.currentContract)) args.push(this.donate_dust)
