	...
	<div>
		<div class='window white'>
			<div class='poolselect'>
				...
				<input id='uSwap' type='checkbox' value='uSwap' v-model='piepools'/>
				<label for='uSwap'>uSwap</label>
		...
		<div class='window white'>
			<select class='tvision' v-model='selectpair'>
				<option v-for='pair in currencies' :value='pair'> {{pair}} </option>
			</select>
...
<script>
	...
	export default {
		...
		data: () => ({
			...
			piepools: ['compound', 'usdt', 'y', 'busd', 'susd', 'pax', 'tbtc', 'ren', 'sbtc', 'hbtc', 'uSwap','3pool'],
			selectpair: 'DAI ⇄ USDC',
			volumes: [],
			pairVolumes: {},
			period: 'week',
		...
		computed: {
			currencies() {
				let pairs = Object.values(allCurrencies).filter(p=>p != 'susd').map(v=>Object.keys(v))
				let allPairs = []
		...
		methods: {
			...
			async mounted() {
				...
				for(let [pair,volume] of Object.entries(this.pairVolumes)) {
			...
			loadPieChart() {
				...
				let filtered = Object.values(this.pairVolumes).map(vol=> {
				...
				filtered
					.forEach((vol, i, arr) => {
						piechartdata.push({
							name: Object.keys(this.pairVolumes)[i],
