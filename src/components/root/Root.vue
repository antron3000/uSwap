	                    ...
	                    <span class='apr'>
	                    	<span>
	                    		...
	                    		<div v-show="CRVAPYs['3pool'] && CRVAPYs['3pool'][0]" :class="{'incentive-apr': true}">+{{CRVAPYs["3pool"] && (CRVAPYs["3pool"][0]).toFixed(2)}}%<!--  <br> to {{CRVAPYs["3pool"] && (CRVAPYs["3pool"][1]).toFixed(2)}}% -->
									...
									<span class='tooltip'>[?]
	                					<span class='tooltiptext long'>
	                						Boosting up to 2.5x will start on 20/09/2020 17:58:29 UTC
	                					</span>
	                				</span>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getAPY() {
				let pools = ['compound', 'usdt', 'y', 'busd', 'susd', 'pax', 'tbtc','ren2','rens','hbtc','uSwap','3pool']
			...
			async getCRVAPY() {
				...
				let pools = ['compound','usdt','iearn','busd','susdv2','pax','ren','sbtc','hbtc','uSwap','3pool']
